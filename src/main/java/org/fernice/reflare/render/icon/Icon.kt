...
object IconPseudoElementHelper {
    ...
    fun getIcon(style: ComputedValues, completion: () -> Unit): Icon? {
        val (url, size) = selectImage(style.background) ?: return null
        ...
        if (iconFuture.isDone && !iconFuture.isCompletedExceptionally) {
            ...
            return if (imageSize != null) {
                ...
                FlareImageIcon(processedIcon.getScaledInstance(width.px().toInt(), height.px().toInt(), AWTImage.SCALE_SMOOTH))
            } else {
                FlareImageIcon(processedIcon)
            }
        }
